...
<script>
  ...
  import loading from '@/common/loading.js'
    ...
    mounted() {
      ...
      // this.getLocation()
      console.log("created")
      // window.location.href = baseURL + "/order/callback/yl_oauth" // 跳转外部链接
      /* 判断过期 */
      ...
      } else if (1/*ua.match(/AlipayClient/i) == 'alipayclient' || ua.match(/CloudPay/i) == 'alipayclient'*/){
        let fromWhere = '';
        if (ua.match(/AlipayClient/i) == 'alipayclient' ){
          fromWhere = 'aLi';
        }else /*if(ua.match(/CloudPay/i))*/{
          fromWhere = 'yinLian';
          // alert('userId:'+this.$route.query.userId);
          window.location.href = baseURL + "/order/callback/yl_oauth?userId="+ this.$route.query.userId  // 跳转外部链接
        }
        let uuid = initParams(this.$route.query.uuid)
        let equipmentId = initParams(this.$route.query.equipmentId)
        this.$router.push({
      ...
      getLocation() { //检测是否支持地理定位 如果支持，则运行 getCurrentPosition() 方法。如果不支持，则向用户显示一段消息。
        if (navigator.geolocation) {//如果getCurrentPosition()运行成功， //则向参数showPosition中规定的函数返回一个coordinates对象
          let gps = navigator.geolocation.getCurrentPosition(showPosition);
          console.log('111111122222222222223', gps)
        } else {
