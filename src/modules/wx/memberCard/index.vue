<template>
	<div class="bg-white">
		...
		<div class="lf-mat lf-mat-f" :style="{height: htmlProportion * 40 + 'px'}"></div>
		<!-- 内容 -->
		<div class="lf-content" :style="{height: htmlheight - (htmlProportion * 200) -1 + 'px'}">
			<div class="lf-content-card">
               ...
               >
                  ...
                  <img v-else src="../../../assets/icon_member_default.png">
					...
					<div class="lf-card-details text-sm" @click="jumpMemberDetails">
						<div>
							<span class="iconfont lf-icon-xingxing text-blue margin-right-10"></span>
							<span>权益</span>
						</div>
						<div class="text-grey">
							<span>查看会员卡详情</span>
              ...
              <div class="display-flex-1">
                <input class="phone-input" type="number" v-model="phone" style="font-size: 0.875rem;padding: 0.3125rem 0.625rem;width: 100%;" placeholder="请输入您的手机号">
              </div>
						</div>
            <!--
						<div class="display-flex">
              ...
              <div class="display-flex-1">
                <input class="lf-code-width margin-right-10 code-input" v-model="code" style="font-size: 0.875rem;padding: 0.3125rem 0.625rem;width: 80%;" type="number" placeholder="输入验证码" maxlength="6">
              </div>
							<div>
                ...
                <span v-else class="lf-code-btn text-sm" :class="{showHighCode:phone}">{{showTime}}s可获取</span>
					...
					<!-- 立即领取按钮 -->
					<div class="lf-card-btn" :class="{showHighCode:code}" :style="{height: htmlProportion * 90 + 'px'}" @click="getMemberCard">
						<span class="text-lg text-white" style="font-size: 1.3rem;">立即领取</span>
					</div>
				</div>
				<div>
					<span @click="jumpMemberServiceAgreement" class="text-blue text-sm">登录即同意《会员服务协议》</span>
				</div>
			</div>
		</div>
		<div class="lf-mat lf-mat-f" :style="{height: htmlProportion * 40 + 'px'}"></div>
		<!-- 错误提示 -->
		<transition name="tip">
			<div class="lf-err-tip" v-if="isErr">{{errTxt}}</div>
		</transition>
	</div>
</template>
...
<script>
  ...
  import bgImgUrl from '@/assets/bg_card.png'
	export default{
		data() {
			return{
      ...
      cardSrc(val,oldVal){
		...
		methods:{
          ...
          if (this.memberDetail.backgroundPictureId) {
            this.cardSrc = this.getPic(this.memberDetail.backgroundPictureId)
          } else {
            this.cardSrc = this.bgImgUrl
          }
				}).catch(err => {
					console.log(err)
				})
...
<style>
  ...
  ..lf-iphone-checking input:focus{
    outline: none;
    outline-offset: 0;
  }
